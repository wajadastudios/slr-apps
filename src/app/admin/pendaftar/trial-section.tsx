...
export type TrialRegistration = {
  ...
  trial_pelatih_id: string | null;
  trial_session_date: string | null;
  trial_session_time: string | null;
  trial_location: string | null;
  payment_token: string | null;
  trial_proof_url: string | null;
  program: { name: string } | null;
};

// The public trial-class registrations (before an account exists). Unchanged
// behaviour; it lives on its own tab of the Pendaftar page.
export function TrialSection({
  ...
  pelatihList: { id: string; full_name: string; title: string | null }[];
  origin: string;
  error?: string;
}) {
    ...
    <div className="flex flex-col gap-6">
      ...
      <GlassCard>
        ...
        <div className="flex flex-col gap-3">
          {pending.length === 0 && (
            <EmptyState title="Tidak ada pendaftar trial baru." hint="Pendaftar dari formulir kelas trial di halaman depan akan muncul di sini." />
          )}
          {pending.map((r) => {
